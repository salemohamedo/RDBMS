...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            //index nested loop num reads = n(o) + n(i)*n(o) 
            //where n is num tuples and r and s are two relations
            //you need to do io for the inner relation n(o) many times
            //and io for outer relation only once
...
/*    /**
 ...
 /*   public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp == Predicate.Op.EQUALS){
            ...
            //get table1 and table2 iterators
            ...
            myTransaction1.start();
            Transaction myTransaction2 = new Transaction();
            myTransaction2.start();
            HeapFile hf1 = (HeapFile)Database.getCatalog().myCatalog.get(table1ID).file;
            DbFileIterator iterator1 = hf1.iterator(myTransaction1.getId());
            HeapFile hf2 = (HeapFile)Database.getCatalog().myCatalog.get(table2ID).file;
            DbFileIterator iterator2 = hf2.iterator(myTransaction2.getId());


            int table1FieldIndex = hf1.getTupleDesc().fieldNameToIndex(field1PureName);
            int table2FieldIndex = hf2.getTupleDesc().fieldNameToIndex(field2PureName);

            Map<Field,Boolean> distinctValuesTable1 = new HashMap<Field,Boolean>();
            Map<Field,Boolean> distinctValuesTable2 = new HashMap<Field,Boolean>();
        ...
        try{
            ...
            while(iterator1.hasNext()){
                ...
                Tuple curTuple = iterator1.next();
                Field curField = curTuple.getField(table1FieldIndex);
                if(distinctValuesTable1.containsKey(curField)){
                    distinctValuesTable1.put(curField,false);
                }
                else{
                    distinctValuesTable1.put(curField,true);
                }
            }

            while(iterator2.hasNext()){
                Tuple curTuple = iterator2.next();
                Field curField = curTuple.getField(table2FieldIndex);
                if(distinctValuesTable2.containsKey(curField)){
                    distinctValuesTable2.put(curField,false);
                }
                else{
                    distinctValuesTable2.put(curField,true);
                }
            }
        }catch(Exception e){
            ...
            int numDistinctTable1 = 0;
            int numDistinctTable2 = 0;

            for(Field f1 : distinctValuesTable1.keySet()){
                boolean distinct = distinctValuesTable1.get(f1);
            ...
            for(Field f2 : distinctValuesTable2.keySet()){
                boolean distinct = distinctValuesTable2.get(f2);
                if(true){
                    numDistinctTable2++;
                }
            }
            ...
            if(numDistinctTable1 > numDistinctTable2){
                selectivity = (double)((double)1/(double)numDistinctTable1);
            }
            else{
                selectivity = (double)((double)1/(double)numDistinctTable2);
            }

            return (int)(card1*card2*selectivity);
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp == Predicate.Op.EQUALS){
            ...
            //no primary keys, return larger of two
            //cardinalities
            else{
                if(card1 > card2){
                    return card1;
     ...
     */
    public Set<BitSet> enumerateBitSubsets(int length, int size) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (BitSet s : els) {
                for (int t : v) {
                    BitSet news = (BitSet) (((BitSet) s).clone());
                    if (news.get(t) == false){
                        news.set(t);
                        newels.add(news);
                    }
    ...
    //convert a given bitset to a sert with the corresponding
    //Logical Joins Nodes
    public Set<LogicalJoinNode> bitsetToNodeSet(BitSet bs){
        Set<LogicalJoinNode> nodes = new HashSet<LogicalJoinNode>();

        for (int i = 0; i < bs.length(); i++) {
             if (bs.get(i)) {
                 nodes.add(joins.get(i)); 
             }
...
11. return optjoin(j)*/
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for(int i = 1; i <= joinsSize; i++){
            for(BitSet bs : enumerateBitSubsets(joinsSize,i)){
                CostCard bestPlan = new CostCard();
                bestPlan.cost = Double.MAX_VALUE;
                bestPlan.card = Integer.MAX_VALUE;
                bestPlan.plan = null;
                Set<LogicalJoinNode> nodeSet = bitsetToNodeSet(bs);
                for(LogicalJoinNode node : nodeSet){
                    CostCard newPlan = computeCostAndCardOfSubplan(stats,filterSelectivities,node,nodeSet,bestPlan.cost,planCache);
                    if(newPlan != null){
                        //found a better plan
                        if(newPlan.cost < bestPlan.cost){
                            bestPlan = new CostCard();
                            bestPlan.cost = newPlan.cost;
                            bestPlan.card = newPlan.card;
                            bestPlan.plan = newPlan.plan;
                        }
                }
                }
                planCache.addPlan(nodeSet,bestPlan.cost,bestPlan.card,bestPlan.plan);
            }

        }
        
        Vector<LogicalJoinNode> orderedNodes = planCache.getOrder(new HashSet<LogicalJoinNode>(joins));

        if(explain){
            printJoins(orderedNodes,planCache,stats,filterSelectivities);
        }
